...
import { storiesAPI, chaptersAPI, storiesLikeAPI } from "@/lib/api";
import { Story, Chapter, ChapterCreate, ChapterUpdate } from "@/types";
import { useAuth } from "@/contexts/AuthContext";
import toast from "react-hot-toast";
import { Book, Plus, Heart, Share2 } from "lucide-react";
...
export default function StoryDetailPage() {
  ...
  const fetchStory = async () => {
    try {
      ...
      setLikeCount(storyResponse.data.likes || 0);
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await storiesLikeAPI.like(storyId);
  ...
  const handleShare = async () => {
    try {
      const response = await storiesLikeAPI.getShareLink(storyId);
